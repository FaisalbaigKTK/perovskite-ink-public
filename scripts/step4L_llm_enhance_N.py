...
WORKSPACE_ID = os.environ.get("ANTHROPIC_WORKSPACE_ID", "")  # required for identity-linked keys
...
def llm_extract(block_text: str) -> dict:
    ...
    # Identity-linked keys require workspace-id header
    if WORKSPACE_ID:
...
def main() -> None:
    ...
    if WORKSPACE_ID:
        print(f"[step4L] Using workspace: {WORKSPACE_ID[:20]}...")
    else:
        print("[step4L] No ANTHROPIC_WORKSPACE_ID set — if you get 400 errors, set it:")
        print("         set ANTHROPIC_WORKSPACE_ID=wrkspc-...")
